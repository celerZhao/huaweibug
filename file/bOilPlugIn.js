...
bOilPlugIn.prototype = {
        ...
        function checkThis (thisInputId, _this) {
            ...
            if (thisVal == '') {
                ...
                if (!_this.regs[argCurr].reg.test(thisVal)) {
                    ...
                    //修改新密码重复
                    if (argCurr == 'reNewPassWordLoose') {//如果是重复密码校验
                        if ($('#' + argCurr).val() != $('#newPassWordLoose').val()) {//判断两次输入的密码是否相同
                            alertMsgFun(_this);
                        }
                    }
    ...
    //二维码图片倒计时
    qrCodeTime: function ( times, duringTimes,_callback) {
        var _this = this;
        //var   thisElement = $('#' + id);

        if (_this.qrWaitTime <= 0) {
            //thisElement.text('刷新').removeClass('disabled active');
            _this.qrWaitTime = 59;
            _callback();
        } else {
            //thisElement.text(_this.qrWaitTime + ' 秒后更新').addClass('disabled active');
            _this.qrWaitTime--;
            setTimeout(function() {
                var date = new Date(), newTimes = date.getTime();
                _this.qrWaitTime = Math.ceil(duringTimes - (newTimes - times)/1000);
                _this.qrCodeTime( times, duringTimes,_callback);
            }, 1000);
        }
    },
    //倒计时
    statusTime: function ( times, duringTimes,_callback) {
        var _this = this;
        //var   thisElement = $('#' + id);

        if (_this.statusWaitTime <= 0) {
            //thisElement.text('刷新').removeClass('disabled active');
            _this.statusWaitTime = 5;
            _callback();
        } else {
            //thisElement.text(_this.qrWaitTime + ' 秒后更新').addClass('disabled active');
            _this.statusWaitTime--;
            setTimeout(function() {
                var date = new Date(), newTimes = date.getTime();
                _this.statusWaitTime = Math.ceil(duringTimes - (newTimes - times)/1000);
                _this.statusTime( times, duringTimes,_callback);
            }, 1000);
        }
    },
    //表单校验弹层
    alertMsg: function (text, thisElement, icon, isClear, timer1) {
        var text = text || '', thisElement = $(thisElement) || null, icon = icon || '', isClear = isClear || '',
            message = '', iconState = '', iconHtml = '', timer1 = timer1 || '';
        ...
        this.middleText('.mask', thisElement, isClear, timer1);
    },
    ...
    clearAlertMsg: function () {
      var _this = this;
        if ($('.mask').length) {
          window.clearTimeout(_this.clearTime1);
            $('.mask').remove();
        }
    },

    //绝对定位垂直居中
    middleText: function (className, thisElement, isClear, timer1) {
        var _this = this, thisEl = $(className), h = thisEl.height();
        thisEl.css('margin-top', -h/2 + 'px');

        var timeTimeout = timer1 || 3500;
    ...
    tips: function (config) {
        ...
        if (typeof o.btnSureCallback == 'function') {
             ...
             if (typeof resObj == 'object' && resObj) {//返回对象

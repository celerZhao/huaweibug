...
(function(root, factory) {
	...
	MobileSelectArea.prototype = {
		init: function(settings) {
			...
			this.settings.default? this.default = 1 : this.default = 0; //0为空,1时默认选中第一项
		...
		f: function(data) {
			...
			if (_this.index !== 0 && _this.value[_this.index - 1] == "0" && this.default == 0) {
				...
				if (item.length > 0 && this.default == 1) {
					...
					if(this.index ==1){
						if(item[0].oil != undefined){
							_this.otherData = item[0].oil;
						}
					}
				}
				for (var j = _this.default, len = item.length; j < len; j++) {
					...
					if(this.index ==1){
						if(item[j].oil != undefined){
							_this.otherData = item[j].oil;
						}
					}
				}
			}
		...
		submit: function() {
			...
			if (this.trigger[0].nodeType == 1) {
				...
				this.trigger.prop("oil",this.oil);
			}
